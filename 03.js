...
function sendDataToWorker(){
    ...
    worker.postMessage({
        N : N,
        dt : dt,
        cannonUrl : document.location.href.replace(/\/[^/]*$/,"/") + "lib/cannon.js",
        positions : positions,
...
function init() {
    ...
    camera.position.set(-Math.cos( Math.PI/5 ) * 10,
                        10,
                        Math.sin( Math.PI/5 ) * 10);
    scene.add( camera );
    ...
    material = new THREE.MeshLambertMaterial( { color: 0x008356 } );
    ...
    // cubes
    var cubeGeo = new THREE.BoxGeometry( 2, 0.05, 1, 1, 1 );
